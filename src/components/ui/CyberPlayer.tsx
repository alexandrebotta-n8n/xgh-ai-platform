...
function PlayerContent() {
  ...
  // Sincronização de Estado com a HeroSection
  const dispatchPlayerState = (playing: boolean) => {
    window.dispatchEvent(new CustomEvent('xgh-player-state', { detail: { playing } }));
  };
  ...
  useEffect(() => {
    ...
    if (trackParam) {
      ...
      if (index >= 0 && index < playlist.length) setCurrentTrackIndex(index);
    }
    ...
    return () => window.removeEventListener('xgh-force-play', handleForcePlay);
  }, [searchParams]);

  const updateTrack = (newIndex: number) => {
    setCurrentTrackIndex(newIndex);
    router.push(`?track=${newIndex + 1}`, { scroll: false });
  };
  ...
  return (
    <div className="w-full relative">
      {/* O brilho (inset-1) foi removido daqui para ficar no pai (HeroSection) */}
      
      {/* AJUSTE DE HARDWARE: 
         - rounded-b-none: Remove arredondamento inferior para colar na discografia.
         - border-b-0: Remove a borda de baixo para não duplicar com o topo da lista.
      */}
      <div className="relative bg-black border-2 border-gray-800 rounded-t-lg rounded-b-none border-b-0 overflow-hidden">
        ...
        <div className="w-full bg-black">
            <div className="flex justify-between px-4 py-1 font-mono text-[9px] uppercase tracking-tighter">
            ...
            <div className="w-full h-1.5 bg-gray-900 cursor-pointer relative" onClick={(e) => {
                if(audioRef.current) audioRef.current.currentTime = (e.nativeEvent.offsetX / e.currentTarget.clientWidth) * audioRef.current.duration;
            }}>
                <div className="h-full bg-neon-green shadow-[0_0_10px_#39ff14]" style={{ width: `${progress}%` }}></div>
            </div>
        </div>

        {/* CONTROLES: MIXER STYLE */}
